...
export class EndScreenModule {
  ...
  generateText (text, size, align, color, forceLato, alpha = 1) {
    var textEl
    if (!forceLato) {
  ...
  createFinisher (finisher) {
    ...
    var turns = finisher.score - finisher.sanity
    var scoreLabel = this.generateText(((turns >= 0) ? turns.toString() + ' turns' + ((finisher.sanity > 0) ? ' + ' + finisher.sanity.toString() + ' sanity' : '') : '-'), 64, 'left', '#ffffff', true, 0.8)
