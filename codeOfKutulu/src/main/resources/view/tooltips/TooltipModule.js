      ...
      if (showing.length) {
        ...
        for (let show of showing) {
          ...
          if (state !== null) {
            ...
            if (String(params.entity).valueOf() == "message") {
              tooltipBlock = params.message;
            } else if (String(params.entity).valueOf() == "minion") {
              tooltipBlock = params.type
                      + '\nId: ' + params.id
              if(""+params.type == "WANDERER") {
                tooltipBlock += '\nLife: ' + params.life + " turn" + (params.life == 1 ? "":"s") +" 'til recall"
                tooltipBlock += '\nTarget : ' + params.target;
              } else if(""+params.type == "SLASHER") {
                tooltipBlock += '\nState: ' + params.state
              }
              if(params.spawn)
                tooltipBlock += '\nSpawn: ' + params.spawn + " turn" + (params.spawn == 1 ? "":"s") +" remaining"

            } else if (String(params.entity).valueOf() == "player") {
              tooltipBlock  = 'Id: ' + params.id;
              tooltipBlock += '\nSanity: ' + params.sanity;
          	  tooltipBlock += params.isolated ? '\nIsolated': '';
              tooltipBlock += params.items ? '\nItems : ' + params.items : '';
              tooltipBlock += params.currentItem ? '\nUsing : ' + params.currentItem : '';
            } else if (String(params.entity).valueOf() == "shelter") {
              tooltipBlock  = 'Shelter ';
              tooltipBlock += '\nEnergy: ' + params.energy;
            } else if (String(params.entity).valueOf() == "map") {
              tooltipBlock  = 'Map ';
              var cellx = Math.floor((+point.x - +params.startx)  / (+params.size * +params.scale));
              var celly = Math.floor((+point.y - +params.starty)  / (+params.size * +params.scale) );
              tooltipBlock += '\ncoord: (' + cellx+', '+celly+')';
            }
